...
citasModel.obtenerCitasSeleccionadas = async () => {
    const query = `
    SELECT c.fecha,
    c.hora,
    c.tb_pacientes_idtb_pacientes AS id_paciente,
    u.nombreUsuario AS nombre_medico,
    m.especialidad,
    c.estado 
    FROM tb_citas c 
    JOIN tb_medicos m 
    ...
    JOIN tb_usuarios u 
    ON m.tb_usuarios_idtb_usuarios = u.idtb_usuarios;`; // Asegúrate de que la tabla "ingresos" exista en tu base de datos
    try {
