...
inventarioModel.obtenerInventarioSeleccionados = async () => {
    const query = 'SELECT m.nombre, m.descripcion, m.FechaIngreso, m.cantidad, m.precioCompra, m.precioVenta, p.nombre AS nombre_proveedor FROM tb_medicamentos m JOIN tb_proveedor p ON m.tb_proveedor_idtb_proveedor = p.idtb_proveedor;'; // Asegúrate de que la tabla "ingresos" exista en tu base de datos
    try {
